...
<table class="protocol-table_col2">
    ...
    <tbody>
    <tr>
        <td>КОМАНДА ХОЗЯЕВ - <?=$answer['match']['match']['t1name']?></td>
        <td>КОМАНДА ГОСТЕЙ  - <?=$answer['match']['match']['t2name']?></td>
        <td>ДАТА МАТЧА - <?=common_dateFromSQL($answer['match']['match']['date'])?> <?=$answer['match']['match']['timeh']?>:<?=$answer['match']['match']['timem']?></td>
    </tr>

</table>
<h3><?=$answer['team']['rus_name']?> - Игроки</h3>
...
<h3><?=$answer['team']['rus_name']?> - Официальные лица</h3>
<table class="matchroster-print_table">
    <colgroup>
        <col/>
        <col/>
...
3.	Все игроки команды проинструктированы, что в течение игры их экипировка должна соответствовать требованиям «Правил игры в футбол» редакцией <?=$answer['compinfo']['yearB']?> года  принятых IFAF   и Регламента и в  случае, если их экипировка пришла в негодность в течение игры, то они обязаны немедленно проинформировать об этом тренерский состав команды.<br/>
